...
import './navbar.scss';
import { useProfileState, useLoadDataState, ThemeContext, LayoutContext } from '../../model/app/context';
        ...
        <div className="navbar__left">
          ...
          <div>
            <span className="navbar__greeting">
              Hello {profileState.status === 'logged-in' && `${profileState.username}`}
            </span>
          </div>
        </div>
        <div className="navbar__right">
          <div className="navbar__icon" onClick={() => getLayout(layout === 'row' ? 'card' : 'row')}>
            {profileState.status === 'logged-in' &&
              (layout === 'row' ? <UilGrid size="30" /> : <UilWindowMaximize size="30" />)}
          </div>
          ...
          >
            {profileState.status === 'logged-in' &&
              (loadDataState === 'pagination' ? <UilArrowDown size="30" /> : <UilListOl size="30" />)}
          </div>

          <div className="navbar__icon" onClick={() => getTheme(theme === 'white' ? 'aquamarine' : 'white')}>
